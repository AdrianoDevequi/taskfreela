...
/**
 * Servidor MCP (Streamable HTTP) do Taskfreela.
 *
 * É isso que permite falar "cria uma tarefa disso aí no Taskfreela" dentro de
 * um chat com o Claude: basta cadastrar
 *
 *     https://www.taskfreela.com.br/api/mcp/<TASKFRELA_API_KEY>
 *
 * como conector/servidor MCP. A chave vai na URL porque a tela de conectores
 * do Claude não permite headers customizados — trate a URL como segredo.
 *
 * Implementado na mão (JSON-RPC 2.0) pra não trazer dependência nova e rodar
 * bem em função serverless.
 */
...
export async function POST(req: Request, { params }: { params: Promise<{ token: string }> }) {
    ...
    if (!checkApiKey(decodeURIComponent(token))) {
